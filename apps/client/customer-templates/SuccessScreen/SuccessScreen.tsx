...
export const SuccessScreen = () => {
	...
	const payment = data?.getCheckoutByID?.payments.find(
			...
			<Box flex={1} height={"100%"}>
				...
				{data ? (
					<VStack p={4} space={2}>
						{data?.getCheckoutByID?.splitType ? (
							<HStack justifyContent={"space-between"}>
								<Text fontSize="lg" fontWeight="bold">
									{t("customerCheckout:splitType")}
								</Text>
								<Text fontSize="lg" fontWeight="bold">
									{t(`customerCheckout:${data?.getCheckoutByID?.splitType}`)}
								</Text>
							</HStack>
						) : null}
						<Divider />
						<HStack justifyContent={"space-between"}>
							<Text fontSize="lg" fontWeight="bold">
								{t("customerCheckout:totalAmount")}
							</Text>
							<Text fontSize="lg" fontWeight="bold">
						...
						<HStack justifyContent={"space-between"}>
							<Text fontSize="lg" fontWeight="bold">
								{t("customerCheckout:amountToBePaid")}
							</Text>
							<Text fontSize="lg" fontWeight="bold">
				...
				<HStack space={"4"} p={4}>
					<Button flex={1} onPress={endSession} _text={{ bold: true }}>
						{t("common:logout")}
					</Button>
					<Button
						_text={{ bold: true }}
