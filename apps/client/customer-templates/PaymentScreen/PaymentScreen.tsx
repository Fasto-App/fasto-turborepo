...
const CheckoutForm = () => {
	...
	const handlePayment = async () => {
		const RETURN_URL = `${process.env.FRONTEND_URL}${customerRoute[
			"/customer/[businessId]/success"
		...
		<>
			<Box alignItems={"flex-end"} px={2}>
				{!clientSession?.getClientSession.tab?.orders?.length ? null : (
					...
					>
						{t("placedOrders", {
							number: clientSession?.getClientSession.tab?.orders?.length,
						})}
					</Button>
				)}
			</Box>
			...
			<VStack padding={6} space={2}>
				<Row
					title="Subtotal"
					value={parseToCurrency(data?.getPaymentInformation.checkout.subTotal)}
				/>
				<Row title="Taxes" value={parseToCurrency(0)} />
				<Row
					title="Service Fee"
					value={parseToCurrency(data?.getPaymentInformation.serviceFee)}
				/>
				<Row
					title="Tip"
					value={parseToCurrency(data?.getPaymentInformation.tip)}
				/>
				<Row
					title="Total"
					value={parseToCurrency(data?.getPaymentInformation.amount)}
					bold
				/>
			</VStack>
			<Center p={"6"}>
...
const StripeWrapper: React.FC = ({ children }) => {
	...
	return (
		<Elements stripe={stripePromise(country)} options={{ clientSecret }}>
			{children}
		</Elements>
	);
};
