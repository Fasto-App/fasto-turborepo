...
export const MenuScreen = () => {
	...
	const { data: clientData, loading: clientLoadingData } =
		useGetClientSessionQuery({
	...
	const inputValue = useMemo(
		() => {
			...
			const { streetAddress, city, stateOrProvince, complement } =
				clientData?.getClientSession.user.address;

			return type === TakeoutDeliveryDineIn.Takeout
				? `${type}`
	...
	useEffect(() => {
		if (selectedCategory && scrollViewRef.current) {
			const index = formatToSectionData.findIndex(
				(item) => item.title._id === selectedCategory,
			);
			// @ts-ignore
		...
		<>
			...
			<SectionList
				...
				onViewableItemsChanged={(info) => {
					if (
						info.viewableItems?.[0]?.section?.title._id &&
						selectedCategory &&
					...
					<MenuItem
						...
						onPress={() => {
							if (typeof businessId !== "string")
								throw new Error("businessId is not a string");
