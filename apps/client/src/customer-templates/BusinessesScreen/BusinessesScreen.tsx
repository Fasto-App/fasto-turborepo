		...
		<Box flex={1} background={"white"}>
			<Box backgroundColor={"pink"} h={"full"}>
				<HStack position={"revert"} justifyContent={"space-between"} p={8}>
					...
					<Center>
						<div
							style={{
								background:
									"linear-gradient(349deg, rgba(227,232,0,1) 0%, rgba(255,102,0,1) 25%)",
								borderRadius: "100%",
							}}
						>
							<NavigationButton
				...
				<FlatList
					...
					ListEmptyComponent={
						loading ? <LoadingTiles /> : error ? <Text>Error</Text> : null
					}
...
const BusinessCard = ({
	...
	return (
		...
		>
			<Link
				href={`${customerRoute["/customer/[businessId]"].replace(
					`[businessId]`,
					_id,
				)}`}
			>
				<HStack flex={1} space={4}>
					<Box>
						...
						>
							<Image
								borderRadius={"md"}
								src={imageUrl ?? "/images/fasto-logo.svg"}
								alt={name}
								resizeMode={"cover"}
							/>
						</AspectRatio>
