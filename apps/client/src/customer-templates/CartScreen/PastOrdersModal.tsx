...
export const PastOrdersList = () => {
	const [selectedTab, setSelectedTab] =
		React.useState<(typeof tabs)[number]>("yourOrders");
	...
	const myOrders = useMemo(
		() =>
			data?.getOrdersBySession.filter(
				(item) => item?.user === clientSession?.getClientSession?.user?._id,
			),
		[clientSession?.getClientSession?.user?._id, data?.getOrdersBySession],
	);
		...
		<>
			{loading ? (
				...
				<FlatList
					...
					data={
						selectedTab === "yourOrders" ? myOrders : data?.getOrdersBySession
					}
										...
										return (
											...
											>
												<Heading
													...
													textAlign={"center"}
													color={
														selectedTab === key ? "primary.500" : "gray.400"
													}
													pb={2}
												>
													{`${t(key)} (${
...
export const PastOrdersTile = (props: PastOrdersTileProps) => {
	...
	return (
		...
		>
			<HStack space={2}>
				<Box>
					<Image size={"xs"} source={{ uri: url }} alt={""} borderRadius={5} />
				</Box>
				<Text alignSelf={"center"} w={70}>
					{name}
				</Text>
			</HStack>
...
export const PastOrdersTileWithoutImage = (
	props: PastOrdersTileNoImageProps,
) => {
	...
	return (
		...
		>
			<HStack space={2}>
				<Text alignSelf={"center"} fontSize={"lg"} w={70}>
					{name}
				</Text>
			</HStack>
			<Text fontSize={"lg"}>{`x${quantity}`}</Text>
			<Text>{price}</Text>
			<Text fontSize={"lg"}>{orderStatus}</Text>
		</HStack>
	);
};
