...
import {
	...
	useCustomerRequestPayFullMutation,
...
export const CheckoutScreen = () => {
	...
	const [generatePaymentIntent, { loading: isPaymentIntLoading }] =
		useGeneratePaymentIntentMutation({
			onCompleted: (
				{ generatePaymentIntent: { clientSecret, paymentIntent, amount } },
				clientOptions,
			) => {
				if (!clientOptions?.variables?.input.payment)
	...
	const payment =
		splitType &&
		data?.getCheckoutByID?.payments.find(
			(payment) => payment?.patron === clientData?.getClientSession.user._id,
		);

	const pay = () => {
		if (!checkoutId || !clientData?.getClientSession.tab?.checkout)
	...
	const navigateToSplit = useCallback(() => {
		if (!checkoutId || !businessId)
			throw new Error("Missing checkoutId or businessId");
	...
	const userAddress = useMemo(() => {
		if (
			clientData?.getClientSession.tab?.type !==
				TakeoutDeliveryDineIn.Delivery ||
			!clientData?.getClientSession.user?.address
		)
			return undefined;

		const { stateOrProvince, city, streetAddress, complement } =
			clientData?.getClientSession.user?.address;
			...
			{userAddress ? (
				<>
					...
					<VStack paddingX={"4"} paddingY={"2"} space={"2"}>
						<Text pb={"2"} bold fontSize={"lg"}>
							Delivery Info
						</Text>
						<Text fontSize={"lg"}>
							{clientData?.getClientSession.user.name}
						</Text>
				...
				<Box>
					...
					<HStack space={"4"} p={4}>
						<Button
							...
							isDisabled={
								!(
									clientData?.getClientSession.tab?.admin ===
									clientData?.getClientSession.user._id
								) ||
								!checkoutId ||
								!clientData?.getClientSession.tab?.checkout
							}
							isLoading={loading}
							onPress={pay}
						>
				...
				<Box>
					<VStack p={4} space={2}>
						...
						<HStack justifyContent={"space-between"}>
							...
							<Text fontSize="lg" fontWeight="bold">
								{parseToCurrency(data?.getCheckoutByID.total)}
							</Text>
						</HStack>
						<Divider />
						<HStack justifyContent={"space-between"}>
							...
							<Text fontSize="lg" fontWeight="bold">
								{parseToCurrency(payment?.amount)}
							</Text>
						</HStack>
						<Divider />
					</VStack>
						...
						<HStack space={2}>
							...
							{selectedTip === "Custom" ? (
								<Input
									...
									fontSize={"lg"}
									textAlign={"right"}
									value={(Number(customTip.toString()) / 100).toLocaleString(
										"en-US",
