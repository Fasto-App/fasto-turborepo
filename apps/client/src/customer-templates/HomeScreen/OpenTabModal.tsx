...
const array1to5 = Array.from({ length: 5 }, (_, i) => i + 1).map((i) => ({
	name: i.toString(),
	_id: i.toString(),
}));
...
export const OpenTabModal = ({
	...
	const [openTabRequest, { loading }] = useOpenTabRequestMutation({
		...
		onCompleted: (data) => {
			...
			if (!data?.openTabRequest || typeof businessId !== "string")
	...
	const handleOpenTab = useCallback(
		async (data: NewTabForm) => {
			const { name, phoneNumber, totalGuests, ...rest } = data;
			const array = typedValues(rest).filter(
				(i) => (i as string).trim() !== "",
			) as string[];
