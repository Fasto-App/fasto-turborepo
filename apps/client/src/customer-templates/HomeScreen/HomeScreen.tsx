...
export const HomeScreen = () => {
	...
	const [
		isTakeoutDeliveryDineInModalOpen,
		setIsTakeoutDeliveryDineInModalOpen,
	] = useState(false);
	...
	const { data: businessInfo, loading: businessInfoLoading } =
	...
	return (
		<Center h={"100%"} backgroundColor={"white"}>
			...
			<VStack space={6} mt={"10"} w={"80%"}>
				<Button
					isDisabled={
						loading || tabData?.getClientSession.request?.status === "Pending"
					}
				...
				<Button
					onPress={() => setIsTakeoutDeliveryDineInModalOpen(true)}
					_text={{ bold: true }}
