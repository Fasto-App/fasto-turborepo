...
> = ({ setModalData, modalData, selectedTab, selectedType }) => {
	...
	const isThereNextPage =
		(data?.getOrdersGroup?.length || 0) < pagination.pageSize;
	...
	const [deleteOrder, { loading: deleteLoading }] =
		useDeleteOrdersGroupDataMutation({
			refetchQueries: [
				{
			...
			onCompleted(data) {
				...
				showToast({
					...
					subMessage: t("itemsWereDeleted", {
						count: data.deleteOrdersGroupData.deletedCount,
					}),
				});
			},
			onError() {
	...
	const selectAll = () => {
		const allSelected = data?.getOrdersGroup?.reduce(
			(accumulator, order) => {
