...
export const percentageSelectData: SelectData[] = percentages.map(
	(percentage) => ({
		_id: percentage,
		value: percentage,
	}),
	...
	devtools(
		subscribeWithSelector((set, get) => ({
			...
			setSplitByPatron: (orders: Order[]) => {
				const splitByPatron = orders.reduce(
					(acc, order) => {
			...
			setCustomInputOnChange: (userId: string, value: string) => {
				...
				const valueOfDiscount =
					selectedDiscount === "Custom" ? customDiscount : percentageOfDiscount;
				const valueOfTip =
					selectedTip === "Custom" ? customTip : percentageOfTip;
				...
				const absoluteTotal =
					total - valueOfDiscount + valueOfTip + serviceFeeValue;

				const customTotalRemaing = absoluteTotal - customSummed;

				if (
					customTotalRemaing <= 0 &&
					(!customSubTotals[userId] || convertedValue > customSubTotals[userId])
				) {
					return;
				}
				...
				if (
					convertedValue >
					customTotalRemaing + (customSubTotals[userId] ?? 0)
				) {
					return set((state) => ({
						customSubTotals: {
							...state.customSubTotals,
...
export const useComputedChekoutStore = () => {
	...
	const percentageOfDiscount = getPercentageOfValue(
		store.total,
		store.discount,
	);
	const percentageOfTip = getPercentageOfValue(store.total, store.tip);
	const valueOfDiscount =
		store.selectedDiscount === "Custom"
			? store.customDiscount
			: percentageOfDiscount;
	const valueOfTip =
		store.selectedTip === "Custom" ? store.customTip : percentageOfTip;
	const absoluteTotal =
		store.total - valueOfDiscount + valueOfTip + store.serviceFeeValue;
	const numberOfSelectedUsers = typedValues(store.selectedUsers).filter(
		Boolean,
	).length;
	const splitEqually =
		numberOfSelectedUsers > 0 ? absoluteTotal / numberOfSelectedUsers : 0;
	let amountPerUser = 0;

	const computedSplitByPatron = typedKeys(store.splitByPatron).reduce(
		(acc, key) => {
