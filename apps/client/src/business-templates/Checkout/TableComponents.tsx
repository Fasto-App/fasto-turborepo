...
const Cell: FC<{ bold?: boolean; isDisabled?: boolean }> = ({
	children,
	bold,
	isDisabled,
}) => {
...
const PaymentCell: FC<{ bold?: boolean; isDisabled?: boolean }> = ({
	children,
	bold,
	isDisabled,
}) => {
...
export const Header = ({
	...
	return (
		<HStack py={2}>
			...
			<Cell bold>{t("patron")}</Cell>
			<Cell bold>{t("subtotal")}</Cell>
			{type === "ByPatron" ? <Cell bold>{t("sharedByTable")}</Cell> : null}
			<Cell bold>{t("feesAndTax")}</Cell>
			<Cell bold>{t("tip")}</Cell>
			<Cell bold>{t("total")}</Cell>
			<Box flex={1} />
		</HStack>
	);
};
	...
	return (
		<HStack h={"10"}>
			...
			{type === "Custom" ? (
				...
				<Cell isDisabled={!isUserSelected || hasUserPaid} key={"subtotal"}>
					{isUserSelected ? subTotal : parseToCurrency(0)}
				</Cell>
			)}
			{type === "ByPatron" ? (
				<Cell
...
export const PaymentTile = (props: {
	...
	const { customer, subtotal, tip, cta, onPress, loading, disable } = props;
	return (
		<HStack
