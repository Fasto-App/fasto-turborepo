...
import {
	...
	SplitType as SplitTypeGen,
	useCustomerRequestSplitMutation,
} from "../../gen/generated";
import { parseToCurrency } from "app-helpers";
import {
	percentageSelectData,
	useCheckoutStore,
...
export const Split = ({ subTotal, tax, payments }: SplitProps) => {
	const {
		total,
		tip,
		discount,
	...
	const { absoluteTotal, tipCalculation, discountCalculation } =
		useComputedChekoutStore();
	...
	const { data: tabData } = useGetTabCheckoutByIdQuery({
		...
		onCompleted: (data) => {
			setSelectedUsers({
				...tabData?.getTabByID?.users?.reduce(
					(acc, user) => {
						acc[user._id] = true;
						return acc;
					},
	...
	const paymentsByUser = payments?.reduce(
		(acc, payment) => {
	...
	const allUsersFromTab = useMemo(
		() => tabData?.getTabByID?.users ?? [],
		[tabData?.getTabByID?.users],
	);
	...
	const allSelectedUsers = typedKeys(selectedUsers).filter(
		(key) => selectedUsers[key],
	);
	const totalNumberOfUsers = areAllUsersSelected
		? allUsersFromTab.length ?? 0
	...
	const split = useMemo(() => {
		return tabData?.getTabByID?.orders.reduce(
			(acc, order) => {
				const subtotal = order?.subTotal ?? 0;
				const isUserSelected =
					areAllUsersSelected || selectedUsers[order?.user ?? ""];
	...
	const splitCustomBillComplete = useCallback(() => {
		...
		const equalShare =
			remainingTotal / (totalNumberOfUsers - customUsers.length); // divide by the number of users without custom subtotals
		...
		<Box flex={1}>
			<VStack w={"full"} minW={"lg"} space={4}>
				{selectedSplitType === "ByPatron" ? (
					<SummaryRow label={t("AllByTable")} value={parseToCurrency(total)} />
				) : null}
				<SummaryRow label={t("subtotal")} value={parseToCurrency(subTotal)} />
				<SummaryRow
					label={t("feesAndTax")}
					value={parseToCurrency((tax ?? 0) * (subTotal ?? 0))}
				/>
					...
					<HStack space={2} alignItems={"self-end"}>
						...
						{selectedDiscount === "Custom" ? (
							<Input
								...
								fontSize={"lg"}
								textAlign={"right"}
								value={(customDiscount / 100).toLocaleString("en-US", {
									style: "currency",
									currency: "USD",
								})}
								onChangeText={(value) => {
					...
					<HStack space={2}>
						...
						{selectedTip === "Custom" ? (
							<Input
								...
								fontSize={"lg"}
								textAlign={"right"}
								value={(customTip / 100).toLocaleString("en-US", {
									style: "currency",
									currency: "USD",
								})}
								onChangeText={(value) => {
			...
			<ScrollView flex={1} w={"full"} showsHorizontalScrollIndicator>
				<Center flex={1}>
					...
					{allUsersFromTab.map((user, index) => {
						...
						const valueOfDiscountPerUser = getPercentageOfValue(
							split?.[user._id]?.subTotal ?? 0,
							discount,
						);
						const valueOfDiscount = getPercentageOfValue(
							split?.table?.subTotal ?? 0,
							discount,
						);
						const tableSubTotal =
							(split?.table?.subTotal ?? 0) - valueOfDiscount;
						const totalPersonalAmount =
							(split?.[user._id]?.subTotal ?? 0) - valueOfDiscountPerUser;

						const userSubTotal =
							selectedSplitType === "Equally"
								? subTotalAmountSharedEqually
								: totalPersonalAmount;

						const wSharedByTable =
							tableSubTotal / totalNumberOfUsers + userSubTotal;
						...
						const total =
							...
							userSubTotal +
							(selectedSplitType === "ByPatron"
								? tableSubTotal / totalNumberOfUsers
								: 0);
						...
						requestSplit({
							variables: {
								input: {
									...
									tip,
									selectedUsers: typedKeys(selectedUsers).filter(
										(key) => !!selectedUsers[key],
									) as string[],
									customSplit,
								},
