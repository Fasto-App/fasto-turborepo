...
const Subscription = ({
	...
	return (
		...
		>
			<HStack pb={2}>
				<Badge w={"24"} colorScheme="success">
					{signedUpSub.status.toUpperCase()}
				</Badge>
			</HStack>
			<Text fontSize={"lg"}>
				{"Tier: "}
				<Text bold>{signedUpSub.tier}</Text>
			</Text>
			<Text fontSize={"lg"}>
				{`${t("start")}: ${format(
					signedUpSub.current_period_start * 1000,
					"PPPP",
			...
			<Text fontSize={"lg"}>
				{`${t("finish")}: ${format(
					signedUpSub.current_period_end * 1000,
					"PPPP",
					getLocale(router.locale),
				)}`}
			</Text>
		...
		<HStack justifyContent={"space-around"} w={"100%"} space={4} p={4}>
			{data?.getSubscriptionPrices.map((price) => (
				<VStack
					...
					flex={1}
					borderColor={
						selectedPrice === price.id ? "primary.500" : "coolGray.400"
					}
				...
				>
					...
					<Text fontSize={"lg"}>
						{parseToCurrency(
							price.unit_amount,
							businessInfo?.getBusinessInformation.country,
						) + `/${t("month")}`}
					</Text>
					<Button
...
export const Subscriptions = () => {
	...
	const [createSub, { loading: createLoading, data: createSubData, reset }] =
		useCreateSubscriptionMutation({
			refetchQueries: [{ query: GetSignUpSubscriptionsDocument }],
			onError(error) {
				showToast({
	...
	const [updateSubscription, { loading: updateSubLoading }] =
		useUpdateSubscriptionMutation({
			refetchQueries: [{ query: GetSignUpSubscriptionsDocument }],
			onError(error) {
				showToast({
	...
	const updateSubWithPrice = (price: string) => () =>
		updateSubscription({
			variables: {
				input: {
	...
	const createSubWithPrice = (price: string) => () =>
		createSub({
			variables: {
				input: { price },
			},
					...
					{signedUpSubs?.getSignUpSubscription ? (
						<>
							...
							<PriceSubscriptions
								...
								selectedPrice={
									signedUpSubs?.getSignUpSubscription.items.data[0].price.id
								}
							/>
