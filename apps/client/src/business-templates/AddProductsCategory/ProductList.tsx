...
import { Box, Heading, Link, Text, FlatList, HStack, Badge } from "native-base";
...
const ProductList = ({
	...
	const setProductValues = useCallback(
		(item) => {
	...
	const renderProductTile = useCallback(
		({ item, index }: { item: Products[number]; index: number }) => {
			if (!item) return null;
		...
		<>
			...
			>
				<HStack justifyContent={"space-between"}>
					<HStack space={2}>
						<Heading flex={1}>{selectedCategory?.name ?? t("all")}</Heading>
				...
				<HStack flex={1} space={4} flexWrap={"wrap"} overflowY={"scroll"}>
					{!products.length ? (
						<Box pt={4}>
