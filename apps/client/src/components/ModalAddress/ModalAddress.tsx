...
import {
	...
	useUpdateCustomerUpdateTabTypeMutation,
	useUpdateTypeAndAddressMutation,
...
export const ModalAddress = ({
	...
	const [updateTabTypeAndAddress, { loading: loadingTypeAndAddress }] =
		useUpdateTypeAndAddressMutation({
			refetchQueries: [{ query: GetClientSessionDocument }],
			...successAndFailure,
		});

	const [updateTabType, { loading: loadingType }] =
		useUpdateCustomerUpdateTabTypeMutation({
			refetchQueries: [{ query: GetClientSessionDocument }],
			...successAndFailure,
		});
	...
	const [createCustomerAddress, { loading: loadingCreateAddress }] =
		useCreateCustomerAddressMutation({
			refetchQueries: [{ query: GetClientSessionDocument }],
			...successAndFailure,
		});
	...
	const onAddressValidation = useCallback(
	...
	const onUpdate = () => {
		...
		if (
			(orderUpdate && !selectedAddress) ||
			orderUpdate === TakeoutDeliveryDineIn["Takeout"]
		) {
	...
	const inputAddressDisabled = useMemo(() => {
		return (
			orderUpdate === TakeoutDeliveryDineIn["Takeout"] ||
			(!orderUpdate && selectedType === TakeoutDeliveryDineIn["Takeout"])
		);
	...
	const shouldUpdateBeEnabled = useMemo(() => {
		...
		const newAddress =
			!!selectedAddress &&
			(orderUpdate === TakeoutDeliveryDineIn["Delivery"] ||
				(!orderUpdate && selectedType === TakeoutDeliveryDineIn["Delivery"]));
				...
				<>
					...
					{!selectedAddress && !userAddress ? (
						<>
							...
							{inputAddress.length >= 4 && data
								? data?.getGoogleAutoComplete.map((local) => (
										<Pressable
											key={local.place_id}
										...
										>
											<HStack alignItems={"center"} p={2} space={2}>
												<Icon type="Location" size={"1.2em"} />
												<Text fontSize={"lg"}>{local.description}</Text>
											</HStack>
										</Pressable>
								  ))
								: null}
			...
			<HStack justifyContent={"space-around"}>
				{typedKeys(TakeoutDeliveryDineIn).map((type) => (
					<Pressable
						flex={1}
						key={type}
						onPress={() => onHeaderPress(TakeoutDeliveryDineIn[type])}
					>
						<Heading
							size={"md"}
							textAlign={"center"}
							pb={2}
							color={
								selectedType === TakeoutDeliveryDineIn[type]
									? "primary.500"
									: "gray.400"
							}
						>
