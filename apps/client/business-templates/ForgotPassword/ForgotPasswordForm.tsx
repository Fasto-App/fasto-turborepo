...
export const ForgotPasswordForm = () => {
	const { control, formState, handleSubmit, reset } = useForgotPasswordHook();

	const { t } = useTranslation(["businessForgotPassword", "common"]);

	const [
		recoverPasswordMutation,
		{ data: response, reset: resetNetwork, loading },
		...
		<Center w="100%" height={"100vh"}>
			...
			>
				<AlertDialog.Content>
					...
					<AlertDialog.Header>
						{t("businessForgotPassword:checkYourEmail")}
					</AlertDialog.Header>
					<AlertDialog.Body>
			...
			<Box safeArea p="2" py="8" w="90%" maxW="600">
				...
				<VStack space={3} mt="5">
					...
					<Link href={businessRoute.login}>{t("common:login")}</Link>
