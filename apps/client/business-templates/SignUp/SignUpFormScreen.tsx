	...
	return (
		<Center w="100%" height={"100vh"}>
			...
			<AlertDialog
				...
				isOpen={data?.requestUserAccountCreation?.ok}
				onClose={resetNetwork}
			>
				<AlertDialog.Content>
					<AlertDialog.CloseButton />
					<AlertDialog.Header>
						{t("businessSignUp:checkYourEmail")}
					</AlertDialog.Header>
					<AlertDialog.Body>{t("businessSignUp:weHaveSent")}</AlertDialog.Body>
					<AlertDialog.Footer>
						<Button
							w={"100"}
			...
			<Box safeArea p="2" py="8" w="90%" maxW="600">
				...
				<VStack space={3} mt="5">
					...
					{formState.errors.email ? (
						<Text color={"red.500"}>{t("common:provideValidEmail")}</Text>
					) : formState.errors.emailConfirmation ? (
						<Text color={"red.500"}>{t("common:makeSureEmailsMatch")}</Text>
					) : error ? (
						<Text color={"red.500"}>{t("common:somethingWentWrong")}</Text>
					) : null}
					<Button
						mt="2"
						bg="primary.500"
