...
import {
  ...
  CheckoutStatusKeys,
  useGetOrdersByCheckoutQuery,
  useDeleteCheckoutDataMutation,
  GetCheckoutsByBusinessDocument,
...
export const BottomOrdersTableWithModal = ({
  setModalData,
  modalData,
}: {
  modalData: OrderStatus;
  setModalData: React.Dispatch<React.SetStateAction<OrderStatus>>;
}) => {
  ...
  const [deleteCheckout, { loading: deleteloading }] =
    useDeleteCheckoutDataMutation({
      refetchQueries: [
        {
      ...
      onCompleted(data) {
        ...
        showToast({
          ...
          subMessage: t("itemsWereDeleted", {
            count: data.deleteCheckoutData.deletedCount,
          }),
        });
      },
      onError() {
  ...
  const selectAll = () => {
    const allSelected = data?.getCheckoutsByBusiness.reduce(
...
const Header: FC<{ loading: boolean }> = ({ children, loading }) => (
  <Heading
    color={loading ? "coolGray.300" : "black˝"}
    textAlign={"center"}
    flex="1"
    size={"md"}
  >
    ...
    <CustomModal
        ...
        <VStack space={"2"} pb={"4"}>
          <Text fontSize={"lg"}>
            ...
            <Badge
              variant={"subtle"}
              colorScheme={
                data?.getOrdersByCheckout.status === "Paid"
                  ? "success"
                  : "yellow"
              }
            >
              {data && t(data.getOrdersByCheckout.status)}
            </Badge>
          </Text>

          <Text fontSize={"lg"}>
            {`${t("date")}: ${format(
              Number(data?.getOrdersByCheckout.created_date || 0),
              "PPpp",
              getLocale(router.locale)
            )}`}
          </Text>
      ...
      ModalFooter={
        <Button.Group space={2} flex={1}>
          ...
          <>
            {data?.getOrdersByCheckout.paid ||
            router.pathname ===
              businessRoute["checkout/[checkoutId]"] ? null : (
              <Button
                flex={1}
                onPress={() => {
                  setIsOpen(false);
              ...
              >
                {`${t("checkout")} ${parseToCurrency(
                  data?.getOrdersByCheckout.subTotal
                )}`}
              </Button>
            )}
          </>
