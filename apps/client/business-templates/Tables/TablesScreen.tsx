...
export const TablesScreen = () => {
	...
	const allTablesFromSpace = useMemo(() => {
		return data?.getSpacesFromBusiness?.reduce(
			(acc, space) => {
			...
			[] as
				| {
						...
						tableNumber: string;
						tab?:
							| { __typename?: "Tab" | undefined; _id: string }
							| null
							| undefined;
				  }[]
				| undefined
	...
	const availableTables = useMemo(() => {
		return allTablesFromSpace?.reduce(
			(acc, table) => {
	...
	const occupiedTables = useMemo(() => {
		return allTablesFromSpace?.filter(
	...
	return (
		<>
					...
					subscribeToRequests={() => {
						subscribeToMoreRequests({
							...
							updateQuery: (prev, { subscriptionData }) => {
								...
								console.log(
									"subscriptionData.data.onTabRequest",
			...
			<OccupiedTabModal
				...
				setIsModalOpen={() =>
					setOccupiedModalData({ isOpen: false, tabId: "" })
				}
			/>
			<NewTabModal
				tableId={newTableModalData.tableId}
				setIsModalOpen={() =>
					setNewTableModalData({ isOpen: false, tableId: "" })
				}
			/>
		</>
	);
