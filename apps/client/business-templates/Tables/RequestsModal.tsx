...
import {
	...
	RequestStatus,
	useAcceptTabRequestMutation,
	useDeclineTabRequestMutation,
} from "../../gen/generated";
...
const refetchQueries = [
	{
		...
		variables: { input: { filterBy: RequestStatus.Pending } },
	},
...
const TileRequest = ({
	...
	return (
		<HStack borderRadius={"md"} borderWidth={1} paddingY={1} paddingX={2}>
			<Box flex={1}>
...
export const RequestsModal = ({
	...
	const onAcceptPress = useCallback(
		(_id: string, selectedTable: string) => {
			accept({
				variables: {
					input: {
						request: _id,
	...
	return (
		<CustomModal
				...
				<>
					{isLoading ? (
						...
						<FlatList
							...
							ListEmptyComponent={() => (
								<Text textAlign="center">{t("noPendingRequests")}</Text>
							)}
						/>
					)}
				</>
			...
			ModalFooter={
				<Button onPress={onClose} flex={1} colorScheme={"gray"}>
					{t("close")}
				</Button>
			}
		/>
