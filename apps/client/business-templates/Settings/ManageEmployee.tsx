...
import {
	DICE_BEAR_INITIALS_URL,
	EmployeeInformation,
	PrivilegesKeysArrayObj,
} from "app-helpers";
import { DevTool } from "@hookform/devtools";
import {
	useDeleteBusinessEmployeeMutation,
	useGetAllEmployeesQuery,
	useManageBusinessEmployeesMutation,
	UserPrivileges,
} from "../../gen/generated";
...
export const ManageEmployee = () => {
	...
	const combinedData = useMemo(() => {
		...
		data?.getAllEmployees?.employees,
		data?.getAllEmployees?.employeesPending,
	]);

	const { control, formState, handleSubmit, reset, setValue, getValues } =
		useManageEmployeeFormHook();
	...
	const onSubmit = useCallback(
		async (data: EmployeeInformation) => {
			await manageEmployee({
				variables: {
					input: {
						...
						privilege:
							UserPrivileges[data.privilege as keyof typeof UserPrivileges],
	...
	const [deleteBusinessEmployee, { loading: loadingDelete }] =
		useDeleteBusinessEmployeeMutation({
			refetchQueries: ["GetAllEmployees"],
		});
	...
	return (
		<Box>
			...
			<HStack alignItems={"center"} space={4}>
				<Heading size={"lg"}>{t("employees")}</Heading>
				<MoreButton onPress={() => setIsModalOpen(true)} />
			</HStack>
			{!combinedData.length ? (
				<VStack>
					<Text pt={5}>{t("startAddingEmployees")}</Text>
					<AddMoreButton onPress={() => setIsModalOpen(true)} empty={true} />
				</VStack>
			) : (
				<ScrollView pt={6}>
						...
						{combinedData.map((employee) => {
							...
							return (
								<EmployeeTile
									...
									picture={
										employee.picture || DICE_BEAR_INITIALS_URL(employee.name)
									}
									ctaTitle={t("edit")}
