...
export const SettingsScreen = () => {
	const [selectedTab, setSelectedTab] =
		useState<ManageTabKeys>("QRcodeAndShare");
	...
	const customerPath = data?.getBusinessInformation._id
		? `${process.env.FRONTEND_URL}/${locale ?? "en"}${customerRoute[
				"/customer/[businessId]"
		  ].replace("[businessId]", data?.getBusinessInformation._id)}`
		: null;
		...
		<>
			<Box nativeID={"section-to-print"}>
				<Center flex={1} h={"100%"}>
					<VStack alignItems={"center"} space={8}>
						<Heading>{t("scanToViewMenu")}</Heading>
						<QRCode value={customerPath || ""} size={300} />
						<Heading>{data?.getBusinessInformation.name}</Heading>
						<NextImage
							src={
								data?.getBusinessInformation.picture ??
			...
			<Box flex={1} shadow={"8"} backgroundColor={"white"}>
				...
				<VStack m={"4"} space={"4"} flex={1}>
					...
					<BottomSection>
						<ScrollView>
							{selectedTab === "manageLocation" ? (
								<ManageBusinessLocation />
							) : selectedTab === "manageAccount" ? (
								<ManageAccount />
							) : selectedTab === "manageEmployees" ? (
								<ManageEmployee />
							) : selectedTab === "manageBusiness" ? (
								<ManageBusiness />
							) : selectedTab === "QRcodeAndShare" ? (
								<QRcodeAndShare />
							) : null}
						</ScrollView>
					</BottomSection>
