...
import {
	...
	useMakeCheckoutFullPaymentMutation,
...
export const PayInFull = ({
	...
	setSelectedOption: (
		value: React.SetStateAction<"splitBill" | "success" | "payFull">,
	) => void;
}) => {
	...
	const allUsersFromTab = useMemo(
		() =>
			data?.getTabByID?.users?.map((user, i) => ({
				_id: user._id,
				value: `${t("person")} ${++i}`,
			})) ?? [],
	...
	return (
		<Center>
			<VStack w={"full"} minW={"lg"} space={4}>
				...
				<SummaryRow
					label={t("feesAndTax")}
					value={parseToCurrency(subTotal ? tax ?? 0 * subTotal ?? 0 : 0)}
				/>
					...
					<HStack space={2} alignItems={"self-end"}>
						<FDSSelect
							...
							setSelectedValue={(string) =>
								setSelectedDiscount(string as Percentages)
							}
						/>
				...
				<HStack justifyContent={"space-between"} px={12}>
					...
					<HStack space={2} alignItems={"self-end"}>
						<FDSSelect
							...
							setSelectedValue={(string) =>
								setSelectedTip(string as Percentages)
							}
						/>
						{selectedTip === "Custom" ? (
							<Input
