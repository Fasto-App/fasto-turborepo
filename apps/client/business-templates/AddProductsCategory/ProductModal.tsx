...
const ProductModal = ({
	...
	const [createProduct, { loading: createProductIsLoading }] =
		useCreateProductMutation({
			onCompleted: () => {
				showToast({
	...
	const [updateProduct, { loading: updateProductIsLoading }] =
		useUpdateProductByIdMutation({
			onCompleted: () => {
				showToast({
	...
	const [deleteProduct, { loading: deleteProductIsLoading }] =
		useDeleteProductMutation({
			onCompleted: () => {
				showToast({
	...
	const ProductFormConfig: SideBySideInputConfig = useMemo(
		() => ({
			...
			category: {
				...
				inputType: "Select",
				array:
					allCategories.map((cat) => ({ name: cat.name, _id: cat._id })) ?? [],
			},
			description: {
				name: "description",
				label: t("description"),
	...
	return (
		<>
			<Modal
				isOpen={showModal}
				onClose={closeModalAndClearQueryParams}
				size={"lg"}
				height={"full"}
			>
					...
					<Modal.Footer borderColor={"white"}>
						<Button.Group space={2} flex={1}>
							...
							<Button
								...
								w={"100px"}
								onPress={handleProductSubmit(onProductSubmit)}
							>
