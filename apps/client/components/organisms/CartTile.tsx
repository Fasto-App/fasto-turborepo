...
export const CartTile = (props: CartTileProps) => {
	const { name, index, price, quantity, url, _id, editable, navegateTo } =
		props;
	const [localQuantity, setLocalQuantity] = React.useState(quantity || 1);

	const { t } = useTranslation("common");

	const [updateItem, { loading: loadingUpdate }] =
		useUpdateItemFromCartMutation({
	...
	const [deleteitem, { loading: deleteLoading }] =
		useDeleteItemFromCartMutation({
			refetchQueries: refetchQueries,
			onCompleted: () => {
	...
	const debounceditemUpdate = useCallback(
		debounce((qnt: number) => {
