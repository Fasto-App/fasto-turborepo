...
const WeeklySchedule = () => {
	const { daysOfTheWeek, setOpenHour, setCloseHour, togggleDay } =
		useScheduleStore(
			(state) => ({
	...
	return (
		<Box py={"4"} px={"2"}>
			<Heading size={"md"}>{t("setStandardHours")}</Heading>
				...
				{typedKeys(daysOfTheWeek).map((day, index) => (
					...
					>
						<HStack space={"4"} flex={1}>
							<FormControl.Label
								isRequired={daysOfTheWeek[day]?.isOpen}
								flex={1}
							>
								{day}
							</FormControl.Label>
						...
						<HStack flex={1} space={"4"} alignItems={"center"}>
							...
							/>
							<Text>{daysOfTheWeek[day]?.isOpen ? t("open") : t("close")}</Text>
						</HStack>
						<Transition isVisible={!!daysOfTheWeek[day]?.isOpen}>
							...
							>
								<HoursInput
									...
									placeholder={"Open"}
								/>
								<Text alignSelf={"center"}>{t("to")}</Text>
								<HoursInput
