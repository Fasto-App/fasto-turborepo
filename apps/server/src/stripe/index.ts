...
if (!process.env.STRIPE_SECRET_KEY || !process.env.STRIPE_SECRET_KEY_BRAZIL) {
	throw ApolloError(
		new Error("Missing Stripe secret key env var"),
		"InternalServerError",
	);
}
...
export const stripeAuthorize = async (accountsParams: AccountParams) => {
	const URL = is_dev
		...
		: process.env.FRONTEND_URL +
				appRoute.customerRoute["/customer/[businessId]"].replace(
					"[businessId]",
					accountsParams.businessId,
				) || undefined;
	...
	try {
		...
		const account = await stripe(accountsParams.country).accounts.create(
			accountParams,
		);
...
export const stripeOnboard = async (
	accountId: string,
	locale: Locale,
	country: "US" | "BR",
) => {
	try {
		const accountLink = await stripe(country).accountLinks.create({
			account: accountId,
			refresh_url:
				process.env.FRONTEND_URL + `/${locale}` + businessRoute.payments,
			return_url:
				process.env.FRONTEND_URL + `/${locale}` + businessRoute.payments,
...
export const confirmPaymentWebHook = async (
	metadata: Metada,
	db: Connection,
) => {
	...
	if (!foundCheckout)
		throw ApolloError(new Error("Check not found."), "BadRequest");
