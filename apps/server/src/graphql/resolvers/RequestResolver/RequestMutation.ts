...
const acceptTabRequest: MutationResolvers["acceptTabRequest"] = async (
	...
	if (!foundAdmin)
		throw ApolloError(new Error("Admin Not Found"), "BadRequest");
	...
	if (table.status !== "Available")
		throw ApolloError(new Error("Table Not Available"), "BadRequest");
	...
	if (numGuests > 0) {
		const allUsers = await User.insertMany(
			new Array(numGuests).fill({ isGuest: true }),
		);
...
const requestJoinTab: MutationResolvers["requestJoinTab"] = async (
	...
	if (!tabAdmin)
		throw ApolloError(new Error("Tab Admin Not Found"), "BadRequest");
	if (!foundTab)
		throw ApolloError(new Error("Request Not Found"), "BadRequest");
...
const createNewTakeoutOrDelivery: MutationResolvers["createNewTakeoutOrDelivery"] =
	...
	onTabRequestResponse: {
		subscribe: withFilter(
			...
			(payload, _, { client }: Context) => {
				...
				if (client) {
					return (
						payload.onTabRequestResponse.requestor?.toString() === client._id ||
						payload.onTabRequestResponse.admin?.toString() === client._id
					);
				}
