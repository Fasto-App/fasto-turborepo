...
const getClientSession = async (
	...
	if (foundUser?.address) {
		...
		if (!foundAddress)
			throw ApolloError(
				new Error("Address not found, but not empty"),
				"NotFound",
			);
...
const getTabRequest = async (
	...
	if (!client) {
		throw ApolloError(
			new Error("invalid client token"),
			"Unauthorized",
			"customer",
		);
	}
...
const getPendingInvitations = async (
	...
	if (!client)
		throw ApolloError(new Error("You're no one"), "Unauthorized", "customer");
