...
const getIsConnected: QueryResolvers["getIsConnected"] = async (
	_,
	_args,
	{ business, db },
) => {
	...
	const account = await stripe(country).accounts.retrieve(
...
const createStripeAccessLink: QueryResolvers["createStripeAccessLink"] = async (
	_,
	_args,
	{ business, db },
) => {
...
//@ts-ignore
const getPaymentInformation: QueryResolvers["getPaymentInformation"] = async (
	par,
	{ input },
	{ db },
) => {
