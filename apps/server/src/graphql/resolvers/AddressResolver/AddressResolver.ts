...
const updateAddress = async (
	...
	return await AddressModel(db).findByIdAndUpdate(input.id, input, {
		new: true,
	});
};
...
const getGoogleAutoComplete: QueryResolvers["getGoogleAutoComplete"] = async (
	parent,
	{ input: { text } },
	{ client, locale },
) => {
	...
	if (text.length < 4)
		throw ApolloError(new Error("Provide 5 chars or more"), "BadRequest");

	try {
		const { data } = await axios.get(autocompleteGoogleUrl, {
			params: {
				input: text,
				radius: "500",
...
const createCustomerAddress: MutationResolvers["createCustomerAddress"] =
	async (parent, { input: { streetAddress, complement } }, { db, client }) => {
		if (!client) throw ApolloError(new Error("No customer"), "Unauthorized");
		...
		const address = await AddressModel(db).create({
			streetAddress:
				data?.result.address?.postalAddress.addressLines[0] ||
...
export { AddressResolverMutation, AddressResolverQuery, AddressResolver };
