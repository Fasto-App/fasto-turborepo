...
const createSubscription: MutationResolvers["createSubscription"] = async (
	par,
	{ input: { price } },
	{ user, db, business },
) => {
	...
	if (!foundUser || !foundBusiness || !foundUser.email) {
		throw ApolloError(
			new Error("User, Email or Business not found"),
			"NotFound",
		);
	}
	...
	if (
		...
		throw ApolloError(
			new Error("Error Creating Subscription"),
			"InternalServerError",
		);
...
// @ts-ignore
const updateSubscription: MutationResolvers["updateSubscription"] = async (
	parent,
	{ input: { price, subscription } },
	{ db, business },
) => {
	const country = await getCountry({ db, business });
	if (!country)
	...
	const foundSubscription =
		await stripe(country).subscriptions.retrieve(subscription);

	if (!foundSubscription)
		throw ApolloError(new Error("Subscription Not Found"), "NotFound");
...
// @ts-ignore
const cancelSubscription: MutationResolvers["cancelSubscription"] = async (
	parent,
	{ input: { subscription } },
	{ db, business },
) => {
	...
	const deletedSubscription =
		await stripe(country).subscriptions.del(subscription);
