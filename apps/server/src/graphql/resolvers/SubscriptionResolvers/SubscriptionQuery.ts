...
//@ts-ignore
const getSignUpSubscription: QueryResolvers["getSignUpSubscription"] = async (
	parent,
	args,
	{ db, user, business },
) => {
...
const getTier: StripeSubscriptionResolvers["tier"] = async (
	parent,
	args,
	{ db, business, user },
) => {
	...
	const plan = pricesResponse.data.find(
		(price) => price.id === parent.items.data[0].price.id,
	);
