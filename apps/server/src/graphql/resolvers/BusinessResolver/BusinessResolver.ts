...
const getAllBusiness = async (
	...
	return await BusinessModel(db)
		.find({ picture: { $ne: null }, address: { $ne: null } })
		.sort({ description: -1 });
};

// @ts-ignore
const getBusinessById: QueryResolvers["getBusinessById"] = async (
	_parent,
	{ input },
	{ db },
) => {
...
const getBusinessInformation: QueryResolvers["getBusinessInformation"] = async (
	...
	if (!foundBusiness)
		throw ApolloError(new Error("Error Findind Business Info"), "BadRequest");
...
const updateBusinessInformation = async (
	...
	try {
		const validateDaysOfTheWeek = await hoursOfOperationSchema.parseAsync(
...
//@ts-ignore
const updateBusinessLocation: MutationResolvers["updateBusinessLocation"] =
	async (_parent, { input }, { db, business }) => {
...
const getAllEmployees = async (
	...
	const employeesPending = await Session.find({
		_id: { $in: foundBusiness?.employeesPending },
	});
...
// @ts-ignore
const manageBusinessEmployees: MutationResolvers["manageBusinessEmployees"] =
	async (parent, args, { business, db, locale }) => {
		if (!business) throw ApolloError(new Error("no business"), "Unauthorized");
		...
		if (_id) {
			if (isPending) {
				...
				if (!foundSession.name || !foundSession.email)
					throw ApolloError(new Error(""), "BadRequest");
		...
		if (foundAsUser) {
			if (foundBusiness.employees?.includes(foundAsUser._id)) {
				throw ApolloError(
					new Error("This user is already an employee"),
					"BadRequest",
				);
			}
...
const deleteBusinessEmployee = async (
	parent: any,
	args: { input: any },
	{ business, db }: Context,
) => {
	...
	if (foundBusiness?.employees?.includes(_id) && foundBusiness?.user !== _id) {
		foundBusiness.employees = foundBusiness.employees.filter(
	...
	if (foundBusiness?.employeesPending?.includes(_id)) {
		foundBusiness.employeesPending = foundBusiness.employeesPending.filter(
			(employee: any) => employee !== _id,
		);
		await foundBusiness.save();
	...
	throw ApolloError(
		new Error("Employee not found or there was an error deleting the them."),
		"BadRequest",
	);
};

const shareQRCode: MutationResolvers["shareQRCode"] = async (
	_parent,
	{ input: { email, file } },
	{ business, db, locale },
) => {
