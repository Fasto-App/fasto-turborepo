...
// @ts-ignore
const getOrderDetailByID: QueryResolvers["getOrderDetailByID"] = async (
	_parent: any,
	{ orderDetailID },
	{ db },
) => {
...
const getAllOrderDetailsByDate = async (
	_parent: any,
	{ input }: any,
	{ db, business }: Context,
) => {
...
const getAllOrderDetailsByOrderID = async (
	_parent: any,
	{ input }: { input: any },
	{ db }: Context,
) => {
...
const getOrdersByTabID = async (
	_parent: any,
	{ input }: { input: any },
	{ db }: Context,
) => {
	const OrderDetail = OrderDetailModel(db);
	const allOrderDetailsByTabID = await OrderDetail.find({
		tab: _parent._id.toString(),
	});
	return allOrderDetailsByTabID || [];
};

// @ts-ignore
const getOrdersBySession: QueryResolvers["getOrdersBySession"] = async (
	_parent,
	args,
	{ db, client },
) => {
...
// @ts-ignore
const getOrdersGroup: QueryResolvers["getOrdersGroup"] = async (
	paren,
	{ input },
	{ db, business },
) => {
...
const getOrderGroupById: QueryResolvers["getOrdersGroupById"] = async (
	...
	const foundOrderGroup = await OrdersGroupModel(db)
		.findById(id)
		.populate("orders");
