...
// Quick Sale
// @ts-ignore
const createOrdersCheckout: MutationResolvers["createOrdersCheckout"] = async (
	parent,
	{ input },
	...
	if (!businessUser?._id)
		throw ApolloError(
			new Error("Business user is not logged in"),
			"Unauthorized",
		);
	...
	try {
		...
		const foundBusiness = await Business.findOne({
			createdByUser: businessUser?.business,
		});
		...
		const orderDetails = await Promise.all(
			parsedInputArray.map(async (parsedInput) => {
				const product = await Product.findOne({ _id: parsedInput.product });

				if (!product)
					throw ApolloError(new Error("Product not found"), "NotFound");
...
// @ts-ignore
const createMultipleOrderDetails: MutationResolvers["createMultipleOrderDetails"] =
	async (_parent, { input }, { db, user: businessUser, business }) => {
		try {
			const parsedInputArray = CreateMultipleOrdersDetail.parse(input);
			...
			if (tab.status !== "Open")
				throw ApolloError(new Error("Tab is not open"), "BadRequest");

			const orderDetails = await Promise.all(
				parsedInputArray.map(async (parsedInput) => {
					const user = await UserModel(db).findOne({ _id: parsedInput.user });
					const product = await ProductModel(db).findOne({
						_id: parsedInput.product,
					});
...
//@ts-ignore
const clientCreateMultipleOrderDetails: MutationResolvers["clientCreateMultipleOrderDetails"] =
	async (_parent, { input }, { db, client }) => {
		console.log("createClientMultipleOrderDetails");

		const foundRequest = await RequestModel(db).findOne({
			_id: client?.request,
		});

		if (!foundRequest)
			throw ApolloError(new Error("No found request"), "NotFound");
		...
		const foundCartItems = await Promise.all(
			input.map(async (item) => {
				...
				if (!foundCartItem)
					throw ApolloError(new Error("Cart item is not valid"), "BadRequest");
		...
		const orderDetails = await Promise.all(
			foundCartItems.map(async (parsedInput) => {
				const user = await UserModel(db).findOne({ _id: parsedInput.user });
				const product = await ProductModel(db).findOne({
					_id: parsedInput.product,
				});
				...
				tab.cartItems = tab.cartItems.filter(
					(item) => item?.toString() !== parsedInput._id.toString(),
				);
...
const updateOrderDetail = async (
	...
	try {
		...
		if (!orderDetail)
			throw ApolloError(new Error("No Order detail"), "NotFound");

		const product = await Product.findOne({ _id: orderDetail.product });
		if (!product) throw ApolloError(new Error("No product"), "NotFound");

		const subTotalUpdated = input.quantity
			? (product?.price || 0) * input.quantity
...
// @ts-ignore
const deleteOrdersGroupData: MutationResolvers["deleteOrdersGroupData"] =
	async (_paren, args, { db, business }) => {
		...
		try {
			...
			const result = await OrdersGroupModel(db).deleteMany({
				_id: { $in: objectIds },
				business,
			});
			return result;
...
//@ts-ignore
const updateOrderGroupData: MutationResolvers["updateOrderGroupData"] = async (
	_par,
	{ input },
	{ db },
) => {
	// find the order group and update
	const foundOrderGroup = await OrdersGroupModel(db).findByIdAndUpdate(
		input._id,
		{ status: input.status },
		{ new: true },
	);
	...
	if (!foundOrderGroup)
		throw ApolloError(new Error("Order Group Not Found"), "NotFound");
