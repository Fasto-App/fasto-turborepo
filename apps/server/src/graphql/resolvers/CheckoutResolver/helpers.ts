...
export async function splitBillCheckForErrors(args: SplitBillArgs) {
  if (!args.client?.business && !args.user?.business) throw ApolloError(
    new Error('No user, nor client'), 'Unauthorized')

  const foundBusiness = await BusinessModel(args.db).findById(args.client?.business || args.user?.business)
  if (!foundBusiness?.stripeAccountId) throw ApolloError(new Error('Business not configured to accept payments'), 'Unauthorized')

  const foundCheckout = await CheckoutModel(args.db).findById(args.input.checkout)
  if (!foundCheckout) throw ApolloError(new Error("No Checkout Found"), 'BadRequest')

  const foundUsers = await UserModel(args.db).find({ _id: { $in: args.input.selectedUsers } })
