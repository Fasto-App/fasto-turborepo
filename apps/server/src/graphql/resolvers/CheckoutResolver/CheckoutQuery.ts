...
// @ts-ignore
export const getCheckoutByID: QueryResolvers["getCheckoutByID"] = async (
	parent,
	{ input },
	{ db, client, user },
) => {
...
// @ts-ignore
export const getCheckoutsByBusiness: QueryResolvers["getCheckoutsByBusiness"] =
	async (_parent, { page = 1, pageSize = 10 }, { db, business }) => {
		if (!business) {
...
// @ts-ignore
export const getOrdersByCheckout: QueryResolvers["getOrdersByCheckout"] =
	async (parent, { input }, { db }) => {
		const Checkout = CheckoutModel(db);
		const checkout = await Checkout.findById(input._id);

		if (!checkout)
			throw ApolloError(new Error("Checkout not found"), "NotFound");
...
export const getPaidCheckoutByDate: QueryResolvers["getPaidCheckoutByDate"] =
	async (par, { input }, { db, user, business }) => {
		...
		if (input.type == DateType.AllTime) {
			const total = dataResult.reduce(
				(accu, current) => accu + current.totalAmount,
				0,
			);
