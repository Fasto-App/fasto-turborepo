...
const makeCheckoutPayment: MutationResolvers["makeCheckoutPayment"] = async (
	...
	if (!foundPayment || !foundCheckout)
		throw ApolloError(new Error("No payment os checkout"), "BadRequest");
...
const makeCheckoutFullPayment: MutationResolvers["makeCheckoutFullPayment"] =
	async (parent, { input }, { db }) => {
		...
		const { patron, paymentMethod, tip, checkout, discount } =
		...
		if (!foundCheckout?.tab)
			throw ApolloError(new Error("Tab not found"), "BadRequest");
		if (foundCheckout.splitType)
			throw ApolloError(new Error("Split type is already set"), "BadRequest");

		if (foundCheckout?.discount === undefined && discount) {
			foundCheckout.discount = discount;
			foundCheckout.total =
				foundCheckout.subTotal -
				getPercentageOfValue(foundCheckout.subTotal, discount);
		...
		if (foundCheckout?.tip === undefined && tip) {
			...
			foundCheckout.total =
				foundCheckout.total + getPercentageOfValue(foundCheckout.subTotal, tip);
			await foundCheckout.save();
		}
		...
		async function verifiePatron(patron: string) {
			...
			if (!foundUser)
				throw ApolloError(new Error("User not found"), "BadRequest");
			return foundUser;
		}

		async function updateTabTableAndRequests() {
			const foundTab = await Tab.findByIdAndUpdate(
				foundCheckout?.tab,
				{
					status: "Closed",
				},
...
// @ts-ignore
const customerRequestPayFull: MutationResolvers["customerRequestPayFull"] =
	async (parent, { input }, { db, client, locale }) => {
		const Checkout = CheckoutModel(db);
		const Payment = PaymentModel(db);
		const User = UserModel(db);
		...
		if (!foundCheckout || !foundUser || !foundBusiness?.stripeAccountId) {
			throw ApolloError(
				new Error("Not enough data to perfom the action"),
				"BadRequest",
			);
		}
		...
		const serviceFeeValue = getPercentageOfValue(
			foundCheckout.subTotal,
			foundCheckout.serviceFee || 0,
		);
...
// @ts-ignore
const customerRequestSplit: MutationResolvers["customerRequestSplit"] = async (
	parent,
	{ input },
	{ db, client, user },
) => {
	try {
		...
		async function updateCheckout() {
			...
			const absoluteTotal =
				foundCheckout.subTotal + tipValue + serviceFeeValue + taxValue;
		...
		switch (input.splitType) {
			case "Equally":
				...
				const serviceFee = getPercentageOfValue(
					totalEqually,
					foundCheckout.serviceFee ?? 0,
				);
			...
			case "ByPatron":
				...
				for (const user of foundUsers) {
					const individualTotal =
						(ordersByPatron[user._id.toString()].subTotal ?? 0) +
						tabTotalPerUser.total;
			...
			case "Custom":
				...
				const total = input?.customSplit?.reduce(
				...
				if (total !== foundCheckout.total) {
					throw ApolloError(
						new Error("The total is not equal to the total of the checkout"),
						"BadRequest",
					);
				}
				...
				for (const split of input.customSplit) {
					const payment = await PaymentModel(db).create({
						...
						serviceFee: Math.ceil(
							foundCheckout.serviceFeeValue / input.customSplit.length,
						),
					});
...
// @ts-ignore
const deleteCheckoutData: MutationResolvers["deleteCheckoutData"] = async (
	paren,
	args,
	{ db, business, user },
) => {
	...
	try {
		...
		const result = await CheckoutModel(db).deleteMany({
			_id: { $in: objectIds },
			business,
		});
		return result;
