...
const createCategory = async (
	...
	if (!isCategoryNameUnique(categories, input.name)) {
		throw ApolloError(
			new Error("Category already exists! Please, Try a different one."),
			"BadRequest",
		);
	}
	...
	if (input.parentCategory) {
		...
		if (!parentCategory)
			throw ApolloError(new Error("Parent category not found"), "NotFound");
	}

	if (input.subCategories) {
		subCategories = await Category.find({ _id: { $in: input.subCategories } });
		if (subCategories.length !== input.subCategories.length)
			throw ApolloError(
	...
	try {
		...
		subCategories?.forEach(async (subCategory) => {
			if (subCategory.parentCategory)
				throw ApolloError(new Error("Don't overide subcategory"), "BadRequest");
...
const getAllCategoriesByBusiness = async (
	_parent: any,
	_: any,
	{ db, business }: Context,
) => {
...
const updateCategory = async (
	...
	if (input.name) {
		const categories = (await businessByID.populate("categories")).categories;
		if (!isCategoryNameUnique(categories, input.name))
			throw ApolloError(new Error("not unique"), "BadRequest");
	}
	...
	if (input.parentCategory) {
		...
		if (!parentCategoryFound)
			throw ApolloError(new Error("parent not found"), "NotFound");
	...
	// Check if this is working properly
	if (input.subCategories) {
		const subCategoriesFound = await Category.find({
			_id: { $in: input.subCategories },
		});
		if (subCategoriesFound.length !== input.subCategories.length)
			throw ApolloError(new Error("No all data was found"), "NotFound");
	...
	subCategories?.forEach(async (subCategory) => {
		if (subCategory.parentCategory)
			throw ApolloError(new Error("no parent"), "BadRequest");
...
const deleteCategory = async (
	...
	await MenuModel(db).updateMany(
		{},
		{ $pull: { sections: { category: category._id } } },
	);
...
const linkCategoryToProducts = async (
	...
	try {
		...
		if (productsToLinkArray.length !== products.length)
			throw ApolloError(new Error(), "NotFound");
...
export { CategoryResolverMutation, CategoryResolverQuery };
